...
from services import signal_extractor, allocation_engine
...
@app.get("/outcomes", response_model=List[schemas.OutcomeCreate])
...
@app.post("/outcomes", response_model=schemas.OutcomeCreate)
...
@app.post("/proofs", response_model=schemas.ProofCreate)
...
@app.get("/outcomes/{outcome_id}", response_model=schemas.OutcomeCreate)
...
@app.get("/proofs/{job_id}", response_model=List[schemas.ProofCreate])
...
@app.post("/plugin/evaluate")
def evaluate(request: schemas.EvaluateRequest, db: Session = Depends(get_db)):
    # Architecture Doc: "Validates requests, orchestrates evaluation pipeline..."
    # We use the data provided in the request, as per the contract.
...
@app.get("/evaluations", response_model=List[schemas.EvaluationSummary])
def get_evaluations(db: Session = Depends(get_db)):
    return crud.get_evaluation_summaries(db)

@app.get("/plugin/status/{job_id}")
...
def submit_feedback(feedback: schemas.FeedbackCreate, db: Session = Depends(get_db)):
    ...
    if feedback.evaluation_id:
        ...
        eval_db = db.query(models.Evaluation).filter(models.Evaluation.id == feedback.evaluation_id).first()
...
@app.get("/admin/signal-weights", response_model=List[schemas.SignalWeightResponse])
...
@app.post("/plugin/suggest-tasks", response_model=List[schemas.TaskSuggestion])
